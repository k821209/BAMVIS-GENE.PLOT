...
## input data
chromosome,left,right = sys.argv[1].split(',')
...
def draw_support(chromosome,left,right,expected_strandness):
    ...
    plt.tight_layout()
    plt.title("%s:%d-%d (%s)"%(chromosome,left,right,expected_strandness))
    plt.gcf().subplots_adjust(right=0.75)
    print('end')
    plt.savefig("%s_%d_%d.svg"%(chromosome,left,right))

draw_support(chromosome,left,right,strand)
